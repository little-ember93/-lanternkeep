...
(() => {
  ...
  const SCENE_INFO = {
    morning: {
      label: "Morning light",
      greeting: "morning",
      image: "./assets/scenes/keep_pond_morning.webp",
      alt: "Morning light over Lanternkeep and its pond"
    },
    afternoon: {
      label: "Afternoon glow",
      greeting: "afternoon",
      image: "./assets/scenes/keep_pond_afternoon.webp",
      alt: "Afternoon light over Lanternkeep and its pond"
    },
    evening: {
      label: "Evening hush",
      greeting: "evening",
      image: "./assets/scenes/keep_pond_evening.webp",
      alt: "Evening light settling over Lanternkeep and its pond"
    },
    night: {
      label: "Night lanterns",
      greeting: "night",
      image: "./assets/scenes/keep_pond_night.webp",
      alt: "Lanterns glowing over Lanternkeep and its pond at night"
    }
  };

  const GROVE_STAGES = [
    { min: 0, label: "Baby grove" },
    { min: 5, label: "Young grove" },
    { min: 15, label: "Flourishing grove" },
    { min: 30, label: "Grand grove" }
  ];
  ...
  const els = {
    ...
    keepSceneArt: byId("keepSceneArt"),
    sceneTimeLabel: byId("sceneTimeLabel"),
    groveSceneStage: byId("groveSceneStage"),
    groveSceneCount: byId("groveSceneCount"),
  ...
  function renderDateLine() {
    const now = new Date();
    const scene = getDaypartInfo(now);
    ...
    els.dateLine.textContent = `${date} · Good ${scene.greeting}`;
  }
  ...
  function renderProgress() {
    ...
    els.world.classList.toggle("fully-tended", active.length > 0 && percentage === 100);
  }
  ...
  function renderWater() {
    ...
    els.world.style.setProperty("--water-progress", String(amount / goal));
  ...
  function renderGratitude() {
    ...
    const leafText = `${count} ${count === 1 ? "leaf" : "leaves"}`;
    const stage = groveStageFor(count);

    els.gratitudeLeafCount.textContent = leafText;
    els.groveSceneCount.textContent = leafText;
    els.groveSceneStage.textContent = stage.label;
  ...
  function renderResidents() {
    const residents = {
      daisy: {
        chip: els.daisyChip,
        image: "./resident_daisy_icon_v132.png",
        label: "Daisy"
      },
      mallow: {
        chip: els.mallowChip,
        image: "./resident_mallow_icon_v132.png",
        label: "Mallow"
      },
      silas: {
        chip: els.silasChip,
        image: "./resident_silas_icon_v132.png",
        label: "Silas"
      },
      reggie: {
        chip: els.reggieChip,
        image: "./resident_reggie_icon_v132.png",
        label: "Reggie"
      }
    };
  ...
  function renderSceneArtwork() {
    const key = getDaypartKey(new Date());
    const scene = SCENE_INFO[key];
    const protectedLabel = state.today.lowEnergy ? " · protected" : "";

    els.world.dataset.daypart = key;
    els.sceneBadge.textContent = `${scene.label}${protectedLabel}`;
    els.sceneTimeLabel.textContent = scene.label;

    if (els.keepSceneArt.getAttribute("src") !== scene.image) {
      els.keepSceneArt.src = scene.image;
    }
    els.keepSceneArt.alt = scene.alt;
  }

  function getDaypartInfo(date) {
    return SCENE_INFO[getDaypartKey(date)];
  }

  function getDaypartKey(date) {
  ...
  function registerServiceWorker() {
    ...
    window.addEventListener("load", () => {
      navigator.serviceWorker.register("./sw-v140.js").catch(() => {
